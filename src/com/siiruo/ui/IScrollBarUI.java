...
public class IScrollBarUI extends BasicScrollBarUI {
	...
	/**
	 * paintThumb 绘制scrollbar里拖动的小box
	 */
	protected void paintThumb(Graphics g, JComponent c, Rectangle thumbBounds) {
		...
		g2.setColor(ColorUtil.COMBOBOX_BOUNDS_COLOR);
	...
	/**
	 * paintTrack 绘制scrollbar里小box的轨迹
	 */
	protected void paintTrack(Graphics g, JComponent c, Rectangle trackBounds) {
		g.setColor(ColorUtil.COMBOBOX_COLOR);
		int x = trackBounds.x;
		int y = trackBounds.y;
		...
		g2.setColor(ColorUtil.COMBOBOX_BOUNDS_COLOR.brighter());
	...
	/**
	 * createIncreaseButton创建滚动条的向上滚动的按钮 不要use JButton button = new JButton();
	 * 而是使用 BasicArrowButton 不然将无法响应点击事件
	 */
	protected JButton createIncreaseButton(int orientation) {
		JButton button = new BasicArrowButton(orientation) {
			public void paint(Graphics g) {
				...
				g2.drawImage((ImageIconUtil.COMBOBOX_ARROW_ICON_INTO).getImage(), -1, 0, null);
			}
		};
	...
	/**
	 * createIncreaseButton创建滚动条的向下滚动的按钮 不要use JButton button = new JButton();
	 * 而是使用 BasicArrowButton 不然将无法响应点击事件
	 */
